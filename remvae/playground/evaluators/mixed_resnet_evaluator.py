...
class ResnetEmbeddingEvaluator:
    ...
    def __evaluate_reconstruction_from_images(self) -> float:
        self.image_model.eval()
        distances = []

        with torch.no_grad():
            for img, _ in self.dataset:
                img = self.__prepare_image(img).to(self.device).unsqueeze(0)
                gen_img, _, _ = self.image_model(img)
                gen_img = self.__prepare_image(gen_img).to(self.device).unsqueeze(0)

                emb_real = self.resnet(img)
    ...
    def __evaluate_reconstruction_from_text(self) -> float:
        self.image_model.eval()
        distances = []

        with torch.no_grad():
            for img, desc in self.dataset:
                img = self.__prepare_image(img).to(self.device).unsqueeze(0)
