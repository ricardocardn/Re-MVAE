...
from playground.readers.FashionMNISTMixedDataset.reader import Reader
...
def evaluate_fashion_mnist_accuracy(image_model, text_model, loader, tokenizer, device, results_dir):
    ...
    }, results_dir, "fashion_mnist_accuracy.json")
...
def evaluate_perplexity(image_model, text_model, loader, tokenizer, device, results_dir):
    ...
    }, results_dir, "mixed_perplexity.json")


def save_evaluation_time(start_time, results_dir):
    ...
    print(f"Evaluation time saved at {time_path} ({elapsed_time:.2f} seconds)")
...
def main():
    ...
    save_evaluation_time(start_time, args["results_dir"])
