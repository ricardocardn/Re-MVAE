...
from playground.readers.mnist_mixed_dataset.reader import Reader  # Cambiado para MNIST
...
def evaluate_mixed_fid(image_model, text_model, loader, device, results_dir):
    ...
    print(f"FID Score for Reconstructed Images: {fid_rec:.2f}")
    print(f"FID Score for Generated Images: {fid_gen:.2f}")
    save_metrics({
        "FID_reconstructed": fid_rec,
        "FID_generated": fid_gen
    }, results_dir, "mixed_fid.json")
...
def evaluate_mnist_accuracy(image_model, text_model, loader, tokenizer, device, results_dir):
    ...
    }, results_dir, "mnist_accuracy.json")
...
def evaluate_perplexity(image_model, text_model, loader, tokenizer, device, results_dir):
    ...
    }, results_dir, "mnist_perplexity.json")
...
def main():
    ...
    evaluate_mnist_accuracy(image_model, text_model, loader, dataset.tokenizer, device, args["results_dir"])
